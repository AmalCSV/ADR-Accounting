<?php
	...
	while($row = $vendorDetailsSearchStatement->fetch(PDO::FETCH_ASSOC)){
		$output .= '<tr>' .
						...
						'<td>' . $row['district'] . '</td>' .
						'<td>' . $row['status'] . '</td>' .
						'<td class="text-right">' . '<button onclick=showEditVendor("'. $row['vendorID'] .'") type="button" class="btn btn-primary btn-sm" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fas fa-edit  fa-sm"></i></button>'. '</td>' .
					'</tr>';
	}
