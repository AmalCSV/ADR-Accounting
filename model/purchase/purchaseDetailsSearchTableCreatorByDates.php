<?php
	...
	$purchaseDetailsSearchSql = " SELECT po.*,
            ...
            ELSE ''
        END AS statusText,
	v.companyName as fullName FROM purchaseorder po inner join vendor v on po.vendorID=v.vendorID where isDeleted = false and orderDate >= :fromDate and orderDate <= :toDate ORDER BY po.purchaseID DESC ";

	$purchaseDetailsSearchStatement = $conn->prepare($purchaseDetailsSearchSql);
	$purchaseDetailsSearchStatement->execute(['fromDate' => $fromDate, 'toDate' => $toDate ]);
